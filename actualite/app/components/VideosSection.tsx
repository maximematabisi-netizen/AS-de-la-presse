...
export default function VideosSection() {
  ...
  return (
    <section className="py-6">
      ...
      <div ref={scrollerRef} className="flex gap-4 overflow-x-auto no-scrollbar py-2">
        {videos.map((raw) => {
          const id = extractId(raw);
          if (!id) return null;
          const m = meta[id];
          ...
          return (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="min-w-[280px] flex-shrink-0 bg-black/5 overflow-hidden rounded shadow-sm relative"
              aria-label={m?.title || `Video ${id}`}
            >
              {m?._notFound ? (
                <div className="p-6 text-center text-sm text-gray-700">Cette vidéo n'est plus disponible.</div>
              ) : (
                <>
                  <img
                    src={thumb}
                  ...
                  <div className="p-3">
                    <div className="text-sm font-medium">{m?.title || 'Vidéo'}</div>
                  </div>
                </>
              )}
            </a>
